...
if ($errors) {
    ...
    header('Location: /frontend/login.php?err=1&username=' . $redirectUsername);
    exit;
}
...
try {
    ...
    if ($user) {
        ...
        if (!$isActiveAccount) {
            $redirectUsername = rawurlencode($username);
            header('Location: /frontend/login.php?err=disabled&username=' . $redirectUsername);
            exit;
        }
...
$redirectUsername = rawurlencode($username);
header('Location: /frontend/login.php?err=invalid&username=' . $redirectUsername);
exit;